...
class TestDeleteGenre:
    def test_delete_genre_from_repositoty(self, mock_genre_repository):
        genre = Genre(name="Romance")
        mock_genre_repository.get_by_id.return_value = genre
        use_case = DeleteGenre(mock_genre_repository)

        use_case.execute(input=DeleteGenre.Input(genre.id))

        mock_genre_repository.delete.assert_called_once_with(genre.id)

    def test_when_genre_does_not_exist_than_raise_exception(
        self, mock_genre_repository
    ):
